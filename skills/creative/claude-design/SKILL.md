...
For mobile screenshot-to-HTML/public artifact jobs, see `references/mobile-screenshot-to-public-html.md` for the verified pattern: vision-read screenshot details, build a self-contained responsive phone-frame HTML, publish under `/var/www/html/<slug>/index.html` when appropriate, and verify local + public HTTP before replying.

For Google Docs-to-public-HTML jobs, see `references/google-doc-to-public-html.md` for the verified pattern: try the public `/export?format=html` endpoint before OAuth, transform the document into a designed artifact rather than raw export when user-facing, publish under `/var/www/html/<slug>/index.html`, and verify the exact public `/index.html` URL before replying.

For user-supplied text documents-to-public-HTML jobs, treat the attached/cache text file as source material, parse the structure into cards/sections rather than dumping raw text, publish under `/var/www/html/<slug>/index.html`, and verify local + public HTTP 200 before replying. See `references/text-document-to-public-html.md` for the verified pattern for long one-line pasted reports, sequential numbered-list parsing, collapsed tables, and Basic Auth/public-share fallback. If the task is an audit/comparison update over multiple AI outputs or prior artifacts, merge **all** known source artifacts into the consolidated page, including the previous HTML/page already created in the thread — do not only ingest the newest attachments. Add explicit source coverage notes (which docs/pages are included, which are excluded, and why), and label uncertain fields such as price/distance as estimated or needs verification. When the user asks to “update all hotels/penginapan” or wants to check manually, expand the artifact to include broad candidates (including official/direct hotel websites outside OTA coverage), keep clickable source links in-row, and use status labels like `Direct site`, `Manual pick`, `CEK ULANG`, `BORDERLINE`, `GUGUR BUDGET`, rather than hiding uncertain entries. If the user later asks “mana html nya”, answer with the direct public URL first and keep it minimal; do not bury the link under tables or explanations.

For script-content dashboard jobs (Meta ads scripts with Hook-Agitate-Bridge-Offer-CTA structure, lane filtering, primary text, and summary tables), use `templates/script-content-dashboard.html` as the starter template. It includes the complete dark-theme CSS, HABOC color coding, lane filter tabs, offer/CTA styling, primary text block, headline badge, guardrail warning for rezeki lanes, summary table, notes section, and print support. Populate the template's placeholder sections (scripts per lane, summary table rows, notes) from the document content.

For pricing/comparison pages requested for readability (e.g. “bikin in rupiah dan jadi HTML”), build a compact decision artifact rather than a raw note: convert currencies with the exchange rate/source/date shown, include plan cards + a per-unit cost table + “cheapest/recommended” callout, publish to `/var/www/html/<slug>.html` or `/var/www/html/<slug>/index.html`, verify public HTTP 200, then reply with the direct public link first. See `references/vendor-pricing-comparisons.md` for the vendor-pricing artifact pattern.

For lightweight finance/reconciliation dashboards built from screenshots or chat-provided period totals, use `references/manual-finance-dashboard.md`: publish a static HTML dashboard quickly, preserve account ownership, store manual budgets per account+period (not globally), support custom cycles like 20→19 or one-off first periods, and verify the public page before replying.

For lodging/travel comparison dashboards where proximity matters (hotel/Airbnb/villa near beach, venue, airport, etc.), use `references/travel-listing-location-audits.md`: do not trust the search area bucket as location truth, add a conservative `Jarak Pantai`/location-risk column, keep raw notes separate from derived risk labels, and update exact rows first when the user provides screenshot/map corrections.
