...
When the user wants analysis results from Hermest to be easier to read, treat phrases like `pakai visual-explainer`, `bikin versi HTML visual`, `jadikan dashboard visual`, or `jangan teks biasa` as a request to generate a self-contained HTML visual page rather than a long chat response.

For Hermes installs, this skill can be installed from the upstream repo by copying `plugins/visual-explainer/` into `~/.hermes/skills/<category>/visual-explainer/` and verifying with `hermes skills list` plus `skill_view(name="visual-explainer")`. Keep the copied directory structure intact (`SKILL.md`, `commands/`, `references/`, `templates/`, `scripts/`).

Recommended response flow for user-facing analysis:
1. Give only a short chat summary.
2. Generate the full analysis as an HTML dashboard/page using this skill.
3. Verify the file exists.
4. If the user is on a messaging platform (Discord/Telegram/etc.), do **not** rely on a local filesystem path as the primary deliverable — they often cannot access it. Prefer, in order: a public hosted HTML link, a native attachment, or a readable PNG/PDF export. If sharing a public link, state whether it is temporary/expiring.
   - **Akbar delivery preference:** when he asks for an HTML/dashboard/visual report, do not downgrade to PDF or screenshot as the main answer unless he explicitly asks. He prefers a directly clickable hosted page. If static hosting is available on his VPS, write the self-contained HTML directly under `/var/www/html/` and give the permanent `https://hermest.gerbangduid.my.id/<filename>.html` link.
   - When Akbar says terse phrases like `jadikan HTML`, `HTML-kan jawabanmu`, or `jadikan html jawaban mu`, treat it as a request to convert the immediately preceding answer/analysis into a polished hosted HTML page, not as a request for more explanation in chat.
   - Verify public delivery with an HTTP status check before claiming it is live. If one client/library returns a misleading 403 but `curl` returns `200`, report the curl-verified public URL rather than blocking delivery on the first client result.
5. For screenshots, verify legibility before sending. Full-page screenshots of long dashboards can be too small/unclear in chat; if unclear, create section-by-section images, PDF, or public HTML link instead.
6. Use this especially for audits, Instagram/account analysis, research summaries, comparisons, status matrices, implementation plans, and any 4+ row / 3+ column structured output.
...
For messaging-app delivery where the user needs a clickable dashboard, also see `references/hosted-html-delivery.md` for the VPS/static-server + Cloudflare quick tunnel fallback pattern.
