...
For 20+ videos, parallel fetching via Python `concurrent.futures` cuts wall-clock time dramatically (51 videos → 22s with 10 workers). See [references/batch-fetch.md](references/batch-fetch.md) for the full pattern.
...
def fetch_transcript(video_id, api_key):
    url = f"https://transcriptapi.com/api/v2/youtube/transcript?video_url={video_id}&format=json&send_metadata=true"
...
# Fetch 50+ videos in parallel (10 workers stays well under 300 req/min)
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
    futures = {ex.submit(fetch_transcript, vid, API_KEY): vid for vid in video_ids}
...
**Pitfalls:**
- `urllib.request` uses `open()` + `read()`, not `requests`. No third-party deps needed.
- Set `send_metadata=true` or you won't get YouTube title/author back.
- Always include `User-Agent: HermesAgent/1.0` header or Cloudflare blocks (403/1010).
- API key from the dashboard `sk_...` is ~46 chars. If 401 on first call, key is wrong — ask user to regenerate from transcriptapi.com dashboard.
- 408 = transient timeout. Retry 1–3 times with 2s exponential backoff.
- 404 = video has no captions/subtitles. Cannot be fixed server-side — skip and mark as unavailable.
- Rate limit: 300 req/min on free tier. 10 workers × 51 videos finishes in ~22s well under this.
- Save intermediate results to JSON so you don't re-fetch on error/retry.
- **Verify saved data**: After batch-fetch, NOT every JSON entry contains a valid transcript. A `transcript` key with an array = success; an `error` or `status: error` key = failure. Some tools may save yt-dlp errors instead of actual transcripts — always check content before treating the file as the source of truth.
- **HTML is the data source for post-fetch analysis**: When you generate an HTML page from the batch results, that page (not the intermediate JSON) is often the most reliable artifact. The HTML embeds full transcript text per entry. If a later task needs to analyze transcripts, extract from the HTML first. See [references/html-transcript-page-debugging.md](references/html-transcript-page-debugging.md) for the robust extraction pattern.
...
- **Test single transcript first** before batch: one call with `send_metadata=true` to validate key and see response shape.
- **Export CSV alongside HTML**: generate CSV with all fields for Google Sheets import — user almost always wants a spreadsheet too.
